...
async function handleAction(nodeId, actionValue, distance) {
    ...
    // --- CDN & UPLINK PRE-LOGIC ---
    if (actionDescriptions[nodeId] && actionDescriptions[nodeId][actionValue]) {
        recordActivity(`CDN UPDATE: ${nodeId} — STATUS: ${actionDescriptions[nodeId][actionValue]}`);
    }
        ...
        if (actionValue === 3){
            ...
            nodes.forEach(n => {
                if (Math.hypot(n.x - targetX, n.y - targetY) < protectionRadius) {
                    n.suppressionTimer = 5; // Set the duration
            n.isInvulnerable = true; // Set the physical block
            n.shieldActive = true; // Set the visual indicator  
                    shieldedCount++;
                }
            });
...
// --- PERSISTENT DEFENSE LOGIC ---
    ...
    try {
        ...
        if (data.status === "success") {
            if (data.cdn_msg) recordActivity(data.cdn_msg);
