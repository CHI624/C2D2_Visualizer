...
// Mapping CDN Node IDs to tactical descriptions for the Activity Record
...
function handleAction(nodeId, actionValue, distance) {
    ...
    .then(data => {
        if (data.status === "success") {
            // --- CDN LOGIC INTEGRATION ---
            // Convert server probability into a percentage for the log
            const riskPercent = (data.prob_high * 100).toFixed(1);
            recordActivity(`CDN ADVISEMENT: ${riskPercent}% Sector Risk detected.`);

            // Update simulation spread based on CDN data
            currentSpreadMultiplier = Math.min(0.08, Math.max(0.02, data.spread_increment || 0.05));
            
            // Shake screen if CDN detects extreme danger
            if (data.prob_high > 0.7) {
                document.body.style.animation = "shake 0.5s";
                setTimeout(() => document.body.style.animation = "", 500);
                recordActivity("WARNING: Critical thermal instability detected by CDN.");
            }
        }
    })
    // --- ACTION LOGIC ---
