...
const BACKEND_SHOW_SEQUENTIAL_FIRES = false; // Set to true to debug/see trail fires
...
function toggleActivityModal() {
    ...
    if (modal.style.display === "block") {
        ...
        logDisplay.style.fontSize = "29px";   // Increases text size
...
function generateNodes() {
    // 1. Define a seed for consistent placement
    const SEED = 12345;
    ...
    for (let i = 0; i < totalNodes; i++) {
        ...
        while (!valid && attempts < 150) {
            rx = (gridCol * cellW) + (seededRandom() * cellW);
            ry = (gridRow * cellH) + (seededRandom() * cellH);
...
const wasAlreadyCompromised = node.isCompromised; // Track previous state
        ...
        ctx.save();
        // NEW: If the node is shielded, it cannot be compromised by fire
    ...
    // VISUAL FEEDBACK: Draw a blue shield/ring around protected nodes
    if (node.isInvulnerable) {
        ctx.beginPath();
        ctx.arc(node.x, node.y, 25 / camera.zoom, 0, Math.PI * 2);
        ctx.strokeStyle = "#00ccff";
        ctx.lineWidth = 4 / camera.zoom;
        ctx.setLineDash([5, 5]); // Dashed line to show it's temporary
        ctx.stroke();
        ctx.setLineDash([]); // Reset dash for other objects
    }
// --- 1. Draw The Node Base ---
        if (node.isCompromised && node.revealed) {
        ...
        ctx.strokeStyle = (node.isCompromised && node.revealed) ? "red" : "white";
...
// 2. NEW: Insignia (Star vs House) and Glow Effect
    ...
    ctx.shadowColor = "#ff0000"; // Golden neon glow
    ctx.fillStyle = "#FFD700";
    ctx.font = `bold ${32 / camera.zoom}px Courier New`; // Larger font for visibility
...
ctx.shadowBlur = 30 / camera.zoom;
    ctx.shadowColor = "#ff0000"; // Golden neon glow    
    ctx.font = `bold ${23 / camera.zoom}px Courier New`;
    ctx.fillText("PRIORITY", 0, -assetSize - (42 / camera.zoom));
    ctx.restore();
    ...
    ctx.translate(-node.x, -node.y); 
        ...
        ctx.textAlign = "center";
        const label = (node.isCompromised && node.revealed) ? "OFFLINE" : (node.type === 'investigation' ? "INT" : "ACT");
...
function draw() {
    ...
    if (elapsed >= MAX_TIME) {
        endGame("Time has been reached!.", true);
        return;
    }
/*
...
*/
// Inside your main draw() loop
fires.forEach(f => {
    // Draw the targeting line only during the "Targeting" turn
    if (f.isCommander && f.jumpState === 1 && f.driftTarget && !f.isMitigated) {
        ctx.save();
        // Visual "Charge" effect: The line gets more opaque as it nears the jump turn
    const progressRatio = f.turnCounter / f.requiredTurns;
    ctx.strokeStyle = `rgba(255, 0, 0, ${0.3 + (progressRatio * 0.6)})`; 
    ctx.lineWidth = (4 + (progressRatio * 4)) / camera.zoom;
    
    // The line stays dashed until the final turn
    if (f.turnCounter < f.requiredTurns - 1) {
        ctx.setLineDash([10, 5]);
    } else {
        ctx.setLineDash([]); // Solid line warns player "Jump is happening next turn!"
    }
    /*
        ctx.strokeStyle = "rgba(255, 0, 0, 0.8)"; 
        ctx.lineWidth = 3 / camera.zoom;
        ctx.setLineDash([10, 5]); // Dashed line to indicate "planning"
        */
        ctx.beginPath();
        ctx.moveTo(f.x, f.y); // Current stagnant position
        ctx.lineTo(f.driftTarget.x, f.driftTarget.y); // Future position
        ctx.stroke();
        
        // Target Node Warning Pulse
        ctx.beginPath();
        ctx.arc(f.driftTarget.x, f.driftTarget.y, 15 / camera.zoom, 0, Math.PI * 2);
        ctx.fillStyle = "rgba(255, 0, 0, 0.2)";
        ctx.fill();
        ctx.restore();
    }

    // Only draw the fire icon if it's revealed
    if (f.revealed && f.radius > 0) {
        f.draw(ctx);
    }
});
...
if (ani.type === 'drift_warning') {
        ...
        ctx.save();
        ctx.setLineDash([10, 5]);
        ctx.strokeStyle = "rgba(255, 60, 0, 1.0)";
...
// --- RENDER THE INVESTIGATION SCAN (FIXED) ---
if (ani.type === 'investigation_scan') {
    ctx.save();
    ctx.beginPath();
    ctx.arc(ani.x, ani.y, ani.radius, 0, Math.PI * 2);
    
    // Use the 'life' property to fade out as it reaches maxRadius
    ctx.strokeStyle = `rgba(0, 204, 255, ${ani.life})`;
    ctx.lineWidth = 4 / camera.zoom;
    ctx.stroke();
    
    // Inner glow
    ctx.fillStyle = `rgba(0, 204, 255, ${ani.life * 0.15})`;
    ctx.fill();
    ctx.restore();
    ...
    // Calculate life based on how close it is to its limit
    // This prevents the "Infinite Sweep" across the map
    if (ani.maxRadius) {
        ani.life = 1.3 - (ani.radius / ani.maxRadius);
    } else {
        ani.life -= 0.5; // Fallback if maxRadius isn't defined
    }
    
    // Remove if it's faded out or reached the scan limit
    if (ani.life <= 0 || (ani.maxRadius && ani.radius >= ani.maxRadius)) {
        activeAnimations.splice(index, 1);
    }
}
    if (ani.type === 'control_line') {
        ...
        ctx.lineWidth = 5;
        ctx.beginPath();
        ctx.arc(ani.x, ani.y, 80, 0, Math.PI * 2);
...
async function handleAction(nodeId, actionValue, distance) {
    ...
    // --- 1. CAPTURE LOCAL DESCRIPTION ---
    // We store this in a variable so we can combine it with the backend data later
    let statusUpdate = "Operational";
    if (actionDescriptions[nodeId] && actionDescriptions[nodeId][actionValue]) {
        statusUpdate = actionDescriptions[nodeId][actionValue];
        // Initial log: Tells the user WHAT they did
        recordActivity(`CDN UPLINK: ${nodeId} initialized. [${statusUpdate}]`);
    }

    // ==========================================
    // STEP 1: EXECUTE PLAYER ACTION FIRST
    // ==========================================
    ...
    if (actionValue === 2) { 
        ...
        activeAnimations.push({ x: targetX, y: targetY, radius: 0, maxRadius: 50, type: 'investigation_scan' });
        ...
        priorityZones.forEach(zone => {
            const distToZone = Math.hypot(zone.x - targetX, zone.y - targetY);
            if (distToZone < 50 && !zone.revealed) {
                zone.revealed = true;
                assetsFound++;
            }
        });
        ...
        // Node Reveal
        nodes.forEach(n => {
            if (Math.hypot(n.x - targetX, n.y - targetY) <= 50) n.revealed = true;
        });

        // Fire Reveal
        fires.forEach(f => {
            if (Math.hypot(f.x - targetX, f.y - targetY) < 50) {
        ...
        recordActivity(firesFound > 0 ? `SCAN REPORT: ${firesFound} hidden thermal signatures revealed.` : `SCAN REPORT: No active fires detected in this sector.`);
        ...
        node.evacuationDeployed = true; 
        evacuations.push({ x: targetX, y: targetY, radius: 75, penaltyApplied: false, sourceNode: node.id });
        activeAnimations.push({ x: targetX, y: targetY, radius: 0, maxRadius: 150, type: 'evac' });
        
    } else if (actionValue === 1 || actionValue === 3) { 
        // SUPPRESSION & CONTROL LINE
        let firesAffected = 0;
        const protectionRadius = 50; 
        let shieldedCount = 0;
        if (actionValue === 3){
            // Control Line Logic
            nodes.forEach(n => {
                if (Math.hypot(n.x - targetX, n.y - targetY) < protectionRadius) {
                    n.suppressionTimer = 5;
                    n.isInvulnerable = true;
                    shieldedCount++;
                }
            });
            recordActivity(`DEFENSE: Control Line at ${nodeId}. Sector nodes shielded.`);
        }
        if (actionValue === 1){
        // Neutralize fires
        fires.forEach(f => {
            let isCaught = (Math.hypot(f.x - targetX, f.y - targetY) < protectionRadius);

            // Intercept Jump
            if (f.isCommander && f.driftTarget) {
                if (Math.hypot(f.driftTarget.x - targetX, f.driftTarget.y - targetY) < 60) {
                    isCaught = true;
                }
            }

            if (isCaught && !f.isMitigated) { 
                f.isMitigated = true;
                f.radius = 0;        
                f.isVisible = false; 
                f.jumpState = 0;     
        ...
        recordActivity(`SUPPRESSION: ${firesAffected} threats neutralized at ${nodeId}.`);
    }
        /*
        activeAnimations.push({ 
            x: targetX, y: targetY, radius: 10, maxRadius: 50, 
            type: actionValue === 3 ? 'control_line' : 'suppression_pulse', life: 1.0 
        });
        */
        recordActivity(`DEFENSE: Sector ${nodeId} secured. ${firesAffected} threats neutralized.`);

        // Time Penalty based on success
        if (firesAffected > 0) {
            let timePenalty = firesAffected >= 7 ? 35000 : firesAffected >= 5 ? 25000 : firesAffected >= 3 ? 15000 : 5000;
    ...
    // ==========================================
    // STEP 2: ADVANCE GAME STATE
    // ==========================================

    closeModal();
    discreteTime += TIME_PER_ACTION;
// --- NEW: SPREAD PROTECTION LOGIC ---
// --- PERSISTENT DEFENSE LOGIC ---
    nodes.forEach(n => {
        if (n.isInvulnerable) {
            // 1. IMMUNITY: If it's already on fire, don't let it go 'offline'
            // This allows the old fire to stay visually, but stops the penalty
            if (n.status === 'offline') {
                n.status = 'online'; 
            }

            // 2. BLOCK NEW COMMANDER JUMPS:
            // Look for any commander fire targetting this specific node
            fires.forEach(f => {
                if (f.isCommander && f.driftTarget && f.driftTarget.id === n.id) {
                    f.driftTarget = null; // Cancel the incoming jump
                    f.jumpState = 0;
                    recordActivity(`DEFLECTED: Control Line at ${n.id} blocked a new ignition attempt.`);
                }
            });
        }
    });
    // Decrement shields
    nodes.forEach(n => {
        if (n.suppressionTimer > 0) {
            n.suppressionTimer--;
            if (n.suppressionTimer === 0) {
                n.isInvulnerable = false;
                recordActivity(`SHIELD EXPIRED: ${n.id} is no longer invulnerable.`);
            }
        }
    });
...
try {
        ...
        if (data.status === "success") {
            ...
            // Assuming prob_high (0.0 to 1.0) represents the risk factor
            // Efficiency = (1 - risk) * 100
            const efficiencyScore = ((1.0 - data.prob_high) * 100).toFixed(1);
            
            // --- COMBINED DATA LOGGING ---
            // This merges the [Description] with the [Actual Model Result]
            recordActivity(`CDN ANALYSIS: ${statusUpdate} complete. Mitigation Efficiency: ${efficiencyScore}%`);
            recordActivity(data.cdn_msg); // This is what puts it in the log
            // If the backend sent a specific message (e.g. "15% reduction"), display that too
            if (data.cdn_msg) {
                recordActivity(`MODEL REPORT: ${data.cdn_msg}`);
            }

            // Apply physical fire reduction if the efficiency was high
            if (data.prob_high < 0.3) {
                fires.forEach(f => { if(!f.isMitigated) f.radius *= 0.8; });
            }
        }
    } catch (error) {
        recordActivity(`CDN ERROR: Analysis packet lost. Sector ${nodeId} integrity unknown.`);
    }
}
...
function startRandomFire(isInitial = false) {
    // 1. Pick a potential target
    const targetNode = nodes[Math.floor(Math.random() * nodes.length)];

    // 2. THE ADJUSTMENT: 
    // If the node is shielded, the fire "fails" to ignite it.
    if (targetNode.isInvulnerable) {
        // We log it so the player knows the shield worked
        recordActivity(`BLOCK: Control Line at ${targetNode.id} prevented a new fire from starting.`);
        return; // Stop the function here so no new fire source is created
    }

    // 3. Otherwise, proceed with creating the fire
    const newFire = new FireSource(targetNode.x, targetNode.y);
    fires.push(newFire);
...
if (nodes.length === 0) return;
        ...
        // Initial seed is visible because it's the starting point
        const seed = new FireSource(targetNode.x, targetNode.y, true); 
        seed.isCommander = true;
        seed.originNode = targetNode; // Track where the jump started
        seed.driftTarget = findNextNodeInPath(targetNode);
        fires.push(seed);
        
        recordActivity(`INTELLIGENCE: Initial fire detected at node ${targetNode.id}.`);
        return;
    }
if (Math.random() < 0.05) { 
const healthyNodes = nodes.filter(n => !n.isCompromised);
        if (healthyNodes.length > 0) {
            const breakoutNode = healthyNodes[Math.floor(Math.random() * healthyNodes.length)];
            const breakout = new FireSource(breakoutNode.x, breakoutNode.y, true);
            breakout.isCommander = true;
            breakout.originNode = breakoutNode;
            breakout.driftTarget = findNextNodeInPath(breakoutNode);
            fires.push(breakout);
            recordActivity(`ALERT: Secondary breakout at node ${breakoutNode.id}.`);
        }
    }
fires.forEach(f => {
    // Only process active commanders
    if (f.isCommander && !f.isMitigated) {
        
        if (f.jumpState === undefined) f.jumpState = 0;
        if (f.turnCounter === undefined) f.turnCounter = 0;

        // STATE 0: TARGET ACQUISITION
        if (f.jumpState === 0) {
            f.driftTarget = findNextNodeInPath(f); 
            if (f.driftTarget) {
                const dx = f.driftTarget.x - f.x;
                const dy = f.driftTarget.y - f.y;
                const dist = Math.sqrt(dx * dx + dy * dy);

                // Determine turns based on distance
                if (dist >= 900) {
                    f.requiredTurns = 4;
                } else if (dist >= 600) {
                    f.requiredTurns = 3;
                ...
                recordActivity(`ANALYSIS: Fire targeting ${f.driftTarget.id}. Estimated transit: ${f.requiredTurns} cycles.`);
            }
        } 
        
        // STATE 1: CHARGING / TRANSIT
        else if (f.jumpState === 1) {
            f.turnCounter++; 

            if (f.turnCounter >= f.requiredTurns) {
                // --- THE FIX: PASS THE BATON ---
                
                // 1. Create the NEW fire at the target node
                const nextNodeFire = new FireSource(f.driftTarget.x, f.driftTarget.y, true);
                nextNodeFire.radius = 40;       
                nextNodeFire.maxRadius = 40;
                nextNodeFire.isSequential = false; 
                nextNodeFire.revealed = false; // Discrete: hidden until scanned
                
                // 2. Give the NEW fire Commander privileges so it continues the chain
                nextNodeFire.isCommander = true;
                nextNodeFire.jumpState = 0;
                
                fires.push(nextNodeFire);
                
                recordActivity(`URGENT: Fire surge has successfully crossed to ${f.driftTarget.id}.`);
                
                // 3. Retire the ORIGINAL fire
                // It stays at its current (x,y) forever as a standard fire
                f.isCommander = false; 
                f.jumpState = 0;
                f.driftTarget = null; 

            } else {
                // Still charging the jump
                const remaining = f.requiredTurns - f.turnCounter;
                recordActivity(`INTEL: Fire front approaching ${f.driftTarget.id} (${remaining} cycles left).`);
            }
        }
    }
});
...
// Helper function to turn a specific node into a moving Commander Fire
function spawnCommanderAtNode(node) {
    const fire = new FireSource(node.x, node.y, false); // false = starts discrete/hidden
    fire.isCommander = true;
    
    // Immediately give it a target so it starts moving toward another node
    fire.driftTarget = findNextNodeInPath(node); 
    fire.pathProgress = 0;
    
    fires.push(fire);
}

function findNextNodeInPath(currentNode) {
    let bestNode = null;
    let highestScore = -Infinity; // We want the node with the highest "Wind Alignment"

    nodes.forEach(node => {
        if (node.id !== currentNode.id && !node.isCompromised) {
            const dx = node.x - currentNode.x;
            const dy = node.y - currentNode.y;
            const dist = Math.sqrt(dx * dx + dy * dy);

            // Limit range so it doesn't jump across the whole map
            if (dist < 600) {
                // 1. Calculate the angle from current fire to this potential node
                const angleToNode = Math.atan2(dy, dx);

                // 2. Compare that angle to the Wind Direction (currentWind.angle)
                // Math.cos of the difference gives a value from 1 (perfectly aligned) to -1 (opposite)
                const windAlignment = Math.cos(angleToNode - currentWind.angle);

                // 3. Score the node: We prefer nodes that are close AND downwind
                // Magnitude (0.30) controls how much the wind matters vs just being the closest node
                const score = (windAlignment * currentWind.magnitude) + (1 / (dist / 100));
...
fires.forEach(f => {
        if (f.isCommander && f.driftTarget) {
            const targetNode = nodes.find(n => n.id === f.driftTarget.id);
            
            // If the target of the jump has a control line, cancel the jump
            if (targetNode && targetNode.isInvulnerable) {
                f.driftTarget = null; // Kill the jump path
                f.jumpState = 0;
                recordActivity(`INTERCEPT: Commander fire jump toward ${targetNode.id} deflected by Control Line.`);
            }
        }
    });
    */
    fires.forEach(f => {
        ...
        if (f.isCommander && f.driftTarget && !f.isMitigated) {
            ...
            f.pathProgress += 45; 
             /*
const trailFire = new FireSource(f.x, f.y, false);
            trailFire.isSubNode = true; 
            trailFire.isSequential = true; // <--- MARK AS SEQUENTIAL
            trailFire.isVisible = BACKEND_SHOW_SEQUENTIAL_FIRES; 
            */
            subFires.push(trailFire);
            if (f.pathProgress < totalDist) {
                ...
                subFire.isSequential = true; // <--- MARK AS SEQUENTIAL
                subFire.isVisible = BACKEND_SHOW_SEQUENTIAL_FIRES;
...
function triggerDiscreteSpread() {
    ...
    fires.forEach(fire => {
        if (fire.isMitigated) return;

        if (fire.isCommander && fire.driftTarget) {
            // 1. Hide the "Head" while it's in transit
            ...
            if (distToTarget > 50) { 
                // 2. Spawn trail fires as INVISIBLE
                const trailFire = new FireSource(fire.x, fire.y, false);
                trailFire.maxRadius = 100;
                trailFire.isVisible = false; // The user won't see these
                newSpotFires.push(trailFire);

                // Move the head forward
                fire.x += Math.cos(angleToTarget) * jumpDistance;
                fire.y += Math.sin(angleToTarget) * jumpDistance;
            } else {
                // 3. TARGET REACHED: Reveal the fire at the node
                recordActivity(`CRITICAL: Fire front has captured Node ${fire.driftTarget.id}.`);
                fire.x = fire.driftTarget.x;
                fire.y = fire.driftTarget.y;
                fire.isVisible = true;   // Now it shows up for the user
                fire.revealed = true;    // Automatically reveal the location
                fire.driftTarget = null; 
            }
        }
    });
...
function openActionModal(node) {
    ...
    let choices = {};
    if (node.type === 'action') {
        choices = { 1: "Suppression", 2: "Investigate Scan" };
    } else {
